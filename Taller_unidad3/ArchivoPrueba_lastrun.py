...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Prueba = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('CondicionesExperimento.xlsx'),
        seed=None, name='Prueba')
    thisExp.addLoop(Prueba)  # add the loop to the experiment
    thisPrueba = Prueba.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisPrueba.rgb)
    if thisPrueba != None:
        for paramName in thisPrueba:
            globals()[paramName] = thisPrueba[paramName]
    
    for thisPrueba in Prueba:
        currentLoop = Prueba
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 1.0 repeats of 'Prueba'
    ...
    # keep track of which components have finished
    FInalizacionComponents = [FinPrueba]
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        if FinPrueba.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > FinPrueba.tStartRefresh + 1-frameTolerance:
                # keep track of stop time/frame for later
